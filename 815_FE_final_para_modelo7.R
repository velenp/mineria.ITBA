...
AgregarVariables  <- function( dataset )
{
  ...
  dataset[ cliente_antiguedad==1 , ctrx_quarter_normalizado := as.numeric(ctrx_quarter * 5) ]
  dataset[ cliente_antiguedad==2 , ctrx_quarter_normalizado := as.numeric(ctrx_quarter * 2)]
  dataset[ cliente_antiguedad==3 , ctrx_quarter_normalizado := as.numeric(ctrx_quarter * 1.2 )]
  ...
  # no funcionan ninguna de las que probé
 # dataset[ , fe1 := as.factor(ifelse( ctrx_quarter>=14,1,0 )) ] #Con menos de 14 transacciones en los ultimos noventa dias
  
  #dataset[ , fe3 := as.factor(ifelse( mean(ctarjeta_debito_transacciones) >= 2,1,0 )) ] 
...
require("data.table")
setwd( "C:/Users/penninivel/Documents/MINERIA" )
...
for (var in varMonetarias) { 
  ...
  dataset[foto_mes == 202009, (var) := lapply(.SD, '*', IPC[length(IPC)] / IPC[20]), .SDcols = var]
  dataset[foto_mes == 202010, (var) := lapply(.SD, '*', IPC[length(IPC)] / IPC[21]), .SDcols = var]
  dataset[foto_mes == 202008, (var) := lapply(.SD, '*', IPC[length(IPC)] / IPC[22]), .SDcols = var]
...
#cargo el dataset
dataset  <- fread( "./datasets/competencia1_historia_2022_ajustadoIPC.csv.gz"  )
...
dir.create( "./exp/FE8150_8/", showWarnings = FALSE )
setwd("./exp/FE8150_8/")   #Establezco el Working Directory DEL EXPERIMENTO
...
#grabo el dataset
fwrite( dataset,
        "dataset_7130_ajustado_lags_FE.csv.gz",
        logical01= TRUE,
        sep= "," )
